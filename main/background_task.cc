...
BackgroundTask::BackgroundTask(uint32_t stack_size)
{
    ...
    background_task_handle_ = xTaskCreateStatic([](void *arg)
        ...
        task->BackgroundTaskLoop(); }, "background_task", stack_size, this, 1, task_stack_, &task_buffer_);
#else
    xTaskCreate([](void *arg)
                {
        BackgroundTask* task = (BackgroundTask*)arg;
        task->BackgroundTaskLoop(); }, "background_task", stack_size, this, 1, &background_task_handle_);
#endif
}
